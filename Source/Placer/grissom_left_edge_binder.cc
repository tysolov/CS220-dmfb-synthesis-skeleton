...
void GrissomLEBinder::place(DmfbArch *arch, DAG *schedDag, vector<ReconfigModule *> *rModules, CoalescingPool *coalescedNodes)
{
    ...
    int TRANSFER_IN_RES = RES_TYPE_MAX + 3;
    int TRANSFER_OUT_RES = RES_TYPE_MAX + 4;
    int numExtraResources = 4; // +4 for inputs, outputs, transfer ins, transfer outs, general

    vector< list<AssayNode*> * > opsByResType; // Operations distinguished by operation types
    list<AssayNode*> storeList;
    for (int i = 0; i <= RES_TYPE_MAX + numExtraResources; i++)
    ...
    for (unsigned i = 0; i < schedDag->getAllNodes().size(); i++)
    {
        ...
        else if (n->GetType() == OUTPUT)
        {
            ...
            opsByResType.at(TRANSFER_IN_RES)->push_back(n);
        else if (n->GetType() == TRANSFER_OUT)
        {
            opsByResType.at(TRANSFER_OUT_RES)->push_back(n);
            if (n->endTimeStep > maxTS)
                maxTS = n->endTimeStep;
        }
    ...
    for (int i = 0; i <= RES_TYPE_MAX; i++)
    {
        ...
        for (unsigned j = 0; j < availRes[i].size(); j++)
        {
            ...
            list<AssayNode *>::iterator end = opsByResType.at(rt)->end();
            for (int k = 0; k < 2; k++)
            {
                for (; it != end; it++)
                {
                    ...
                    if (k || n->GetStartTS() >= lastEnd) {
                        ...
                        lastEnd = n->GetEndTS();
                        n->status = BOUND;
                        n->reconfigMod = new ReconfigModule(fm->getResourceType(), fm->getLX(), fm->getTY(),
                                                            fm->getRX(), fm->getBY());
                ...
                it = opsByResType.at(BASIC_RES)->begin();
                end = opsByResType.at(BASIC_RES)->end();
            }
    ...
    // storage-holder scheduled nodes. Assumes binder above will
    ...
    /////////////////////////////////////////////////////////////
    // Now do simple Left-Edge binding for inputs/outputs
    bindTransferIns(opsByResType.at(TRANSFER_IN_RES));
    bindTransferOuts(opsByResType.at(TRANSFER_OUT_RES));
    ...
    // Sanity check: All nodes should be bound by now
    {
        ...
        for (unsigned i = 0; i < schedDag->allNodes.size(); i++)
        {
            ...
            {
                msg << "Unbound Node: ";
                msg << schedDag->allNodes.at(i)->GetSummary(); // Debugging
                msg << endl;
                allBound = false;
            }
        }
