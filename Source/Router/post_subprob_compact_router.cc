...
{
	// If cellType has already been declared, no need to re-delcare b/c it won't change
	// no matter how many assays are routed (dependent upon the architecture)
	if (!cellType)
	{
...
void PostSubproblemCompactionRouter::equalizeGlobalRoutes(map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	for (; dropIt != routes->end(); dropIt++)
	{
		if (!dropIt->second->empty())
		{
			...
			if (!(lrp->dStatus == DROP_OUTPUT || lrp->dStatus == DROP_MERGING || lrp->dStatus == DROP_TRANSFER_IN_MERGE))
			{
				for (unsigned c = lrp->cycle+1; c < routeCycle; c++)
				{
					...
					// If droplet was going to transfer out, mark only the last routing piont as so
					// (mark prior to last as waiting)
					if (lrp->dStatus == DROP_TRANSFER_OUT)
					{
						lrp->dStatus = DROP_WAIT;
						nrp->dStatus = DROP_TRANSFER_OUT;
					}
					else
...
void PostSubproblemCompactionRouter::mergeNodeDroplets(AssayNode *n, map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	bool transferDropThatHasNotMoved = false;
	for (unsigned d = 0; d < n->GetDroplets().size(); d++)
	{
		...
		if (!route->empty() && route->back()->dStatus == DROP_TRANSFER_IN)
			arrivalTimes.push_back(route->back()->cycle);
		else
		{
			for (int r = route->size()-1; r >= 0; r--)
			{
				...
				else if (r == 0 && route->at(r)->dStatus == DROP_TRANSFER_IN)
					transferDropThatHasNotMoved = true; // Droplet was transferred in and remained here since, hence no change in location
			}
		}
	}

	// If one droplet's arrival time was found, and the other has remained in the module input (in storage)
	// since its transfer-in, then we'll say it arrived at the same time as the other droplet
	if (arrivalTimes.size() == 1 && transferDropThatHasNotMoved)
		arrivalTimes.push_back(arrivalTimes.front());
	...
	// If a droplet was just transferred in but is merging, will need to mark it as so to
	// handle properly later
	for (unsigned d = 0; d < n->GetDroplets().size(); d++)
	{
		Droplet *drop = n->GetDroplets().at(d);
		RoutePoint *rp = (*routes)[drop]->back();
		if (rp && rp->dStatus == DROP_TRANSFER_IN && d != minDropletIdIndex)
			rp->dStatus = DROP_TRANSFER_IN_MERGE;
	}
	...
	for (unsigned d = 0; d < n->GetDroplets().size(); d++)
	{
		...
		if (d != minDropletIdIndex)
		{
			...
			int begCycle = -1;
			int endCycle = -1;
			if (lastRp->dStatus != DROP_TRANSFER_IN && lastRp->dStatus != DROP_TRANSFER_IN_MERGE)
			{
			...
			for (int r = continuingDropletRoute->size()-1; r >= 0; r--)
			{
				...
				// If the route we're keeping is a droplet trasnfer in, then, there is
				// no history....so don't continue
...
void PostSubproblemCompactionRouter::processFixedPlaceTS(map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	for (unsigned i = 0; i < thisTS->size(); i++)
	{
		...
		if (!(n->GetType() == DISPENSE || n->GetType() == OUTPUT || n->GetType() == TRANSFER_IN || n->GetType() == TRANSFER_OUT))
		{
			...
			if (n->GetType() == MIX)
			{
				...
				else if (child->GetType() == TRANSFER_OUT)
					exitY = rm->getBY();
				else
				{
			...
			else if (n->GetType() == HEAT || n->GetType() == COOL || n->GetType() == DETECT)
			{
				Droplet *drop = n->GetDroplets().back();
				lrp = (*routes)[drop]->back(); // Last route point
				ResourceType rt;
				if (n->GetType() == HEAT || n->GetType() == COOL)
					rt = H_RES;
				else
					rt = D_RES;
...
//
// Heats/cools/detects will zig-zag through their module until they find a cell
			...
			if (n->GetType() == MIX)
			{
				...
				while (tsCycle < cycle + cyclesPerTS)
				{
					...
					if (modWidth == 1 || modHeight == 1)
					{
                        ...
                        if (nrp->x < 0 || nrp->y < 0) {
                            cout << "BREAK" << endl;
                            cout << "lrp: " << lrp->x << "," << lrp->y << " -- c=" << lrp->cycle << endl;
                            cout << "nrp: " << nrp->x << "," << nrp->y << " -- c=" << nrp->cycle << endl;
                        }
						lrp = nrp;
					}
...
string printType(DropletStatus ds){
    switch (ds) {
        case DROP_TRANSFER_IN:
            return "t_in";
        case DROP_TRANSFER_OUT:
            return "t_out";
        case DROP_WAIT:
            return "wait";
        case DROP_NORMAL:
        ...
        case DROP_MERGING:
            return "merge";
        case DROP_TRANSFER_IN_MERGE:
            return "t_in_merge";
        default:
            return "other::" + to_string(ds);
    }
}
...
void PostSubproblemCompactionRouter::addSubProbToGlobalRoutes(vector<Droplet *> *subDrops, vector<vector<RoutePoint *> *> *subRoutes, map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	while (!subDrops->empty()) {
        ...
        while (!sr->empty()) {
            ...
            if (rp == NULL)
            {
                continue;  //TODO the delays and drop_transfer_in stalls from compaction are causing issues with cycles, debug
                delay++;
                if (!(*routes)[d]->empty()) {
            ...
            else if (rp->dStatus == DROP_TRANSFER_IN && delay > 0)
            {

                // Droplet was transferred-in, so the beginning stalls from compaction
                // need to be added in here.....
//                RoutePoint *nrp = new RoutePoint();
//                nrp->cycle = rp->cycle + delay++;
//                nrp->dStatus = DROP_TRANSFER_IN;
//                nrp->x = (*routes)[d]->front()->x;
//                nrp->y = (*routes)[d]->front()->y;
//                (*routes)[d]->push_back(nrp);
//                int size = delay;
                for (int i = 0; i < delay; i++) {
                    RoutePoint *nrp = new RoutePoint();
                    nrp->cycle = rp->cycle + i;
                    nrp->dStatus = i == 0 ? DROP_TRANSFER_IN : DROP_WAIT;
                    nrp->x = rp->x;
                    nrp->y = rp->y;
                    (*routes)[d]->push_back(nrp);
            }
                ...
                //TODO for now, if a transfer in is encountered, we simply add a new routing point
                //  directly after the transfer in, as the visualizer is not showing these droplets.
                //  this is currently taking over for the cycle issues introduced above
            {
                delay++;
                RoutePoint *nrp = new RoutePoint();
                nrp->cycle = rp->cycle + 1;
                nrp->dStatus = DROP_TRANSFER_IN;
                nrp->x = rp->x;
                nrp->y = rp->y;
                (*routes)[d]->push_back(rp);
...
//        for (RoutePoint *rp : *(*routes)[d]) {
//        	cout << "\t" << rp->cycle << "[" << printType(rp->dStatus) << "]: (" << rp->x << "," << rp->y << ")";
//        }
//        cout << endl;
...
void PostSubproblemCompactionRouter::eliminateSubRouteDependencies(map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	for (unsigned i = 0; i < thisTS->size(); i++)
		if (thisTS->at(i)->GetType() != DISPENSE &&
		    thisTS->at(i)->GetStartTS() == startingTS)
	...
	for (unsigned i = 0; i < routableThisTS.size(); i++)
	{
		...
		for (unsigned p = 0; p < n->GetParents().size(); p++)
		{
            ...
            else if (par->GetType() == TRANSFER_IN) //do not have information on routing points
                continue;
//			else if (par->GetChildren()[0]->GetType() == TRANSFER_OUT)
//            {
//			    s->x = par->GetReconfigMod()->getLX();
//			    s->y = par->GetReconfigMod()->getBY();
//            }
			else
			{
            ...
            {
			    t->x = n->GetParents()[0]->GetReconfigMod()->getLX();
			    t->y = n->GetParents()[0]->GetReconfigMod()->getBY();
            }
			else if (n->GetType() == TRANSFER_IN)  //we don't know where this is coming from
//            {
//			    t->x = n->GetChildren()[0]->GetReconfigMod()->getLX();
//			    t->y = n->GetChildren()[0]->GetReconfigMod()->getBY();
//            }
                continue;
...
void PostSubproblemCompactionRouter::route(DAG *dag, DmfbArch *arch, vector<ReconfigModule *> *rModules, map<Droplet *, vector<RoutePoint *> *> *routes, vector<vector<int> *> *pinActivations, vector<unsigned long long> *tsBeginningCycle)
{
	...
	while(!nodes.empty()) {
        ...
        while (j < nodes.size() && nodes.at(j)->GetStartTS() <= startingTS && nodes.at(j)->GetEndTS() >= startingTS &&
               j < nodes.size())
        ...
        ///////////////////////////////////////////////////////////////////////
        // First create any new droplets for Dispenses or TransferIns
        ///////////////////////////////////////////////////////////////////////
        for (unsigned i = 0; i < thisTS->size(); i++) {
            AssayNode *n = thisTS->at(i);
            if (n->GetStartTS() == startingTS) {
                if (n->GetType() == DISPENSE) {
                    ...
                    n->addDroplet(d);
                } else if (n->GetType() == TRANSFER_IN) {
                    // This method will only be called when routing is being performed for normal assays and not
                    // routing assays that are just connecting the transfer-in and transfer-out nodes, so just
                    // create a new droplet here and add to the node
                    Droplet *d = new Droplet();
                    (*routes)[d] = new vector<RoutePoint *>();
                    n->addDroplet(d);
                    d->volume = 1;
                    d->uniqueFluidName = "1 parts TRANSFER-FLUID";
                }
            }
...
//			}
        ...
        if (!(getPastPlacerType() == GRISSOM_LE_B || getPastPlacerType() ==
                                                     GRISSOM_PATH_B))// || getPastPlacerType() == KAMER_LL_P))// || getPastPlacerType() == NSGAII_P
            eliminateSubRouteDependencies(
                    routes); // Optional; ensures that no source is in the IR of a target (moves the source out of way)
        ...
        addSubProbToGlobalRoutes(subDrops, subRoutes, routes); // Add sub-routes to routes for entire simulation
