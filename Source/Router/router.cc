...
void Router::compactRoutes(vector<Droplet *> *subDrops, vector<vector<RoutePoint *> *> *subRoutes)
{
	...
	for (unsigned i = 0; i < subRoutes->size(); i++)
		if (subRoutes->at(i)->size() > 0)
			if (subRoutes->at(i)->back()->dStatus != DROP_OUTPUT && subRoutes->at(i)->back()->dStatus != DROP_TRANSFER_OUT)
		...
		while (j != cyclesToCheck && subRoute->size() > 0)
		{
			...
			if (rp)
			{
				...
				for (int k = 0; k < i; k++)
				{
					...
					if (prp && doesInterfere(rp, prp) && !(doesInterfere(rp, destPt) && (prp->dStatus == DROP_WAIT || prp->dStatus == DROP_TRANSFER_OUT)))
						isInterference = true;
					if (prpLc && doesInterfere(rp, prpLc) && !(doesInterfere(rp, destPt) && (prpLc->dStatus == DROP_WAIT || prpLc->dStatus == DROP_TRANSFER_OUT))) // DTG, is this if-statement necessary??
						isInterference = true;

					if (prp && rpNc && doesInterfere(rpNc, prp) && !(doesInterfere(rpNc, destPt) && (prp->dStatus == DROP_WAIT || prp->dStatus == DROP_TRANSFER_OUT)))
						isInterference = true;
					if (prpNc && doesInterfere(rp, prpNc) && !(doesInterfere(rp, destPt) && (prpNc->dStatus == DROP_WAIT || prpNc->dStatus == DROP_TRANSFER_OUT)))
						isInterference = true;
					if (prp && rpLc && doesInterfere(rpLc, prp) && !(doesInterfere(rpLc, destPt) && (prp->dStatus == DROP_WAIT || prp->dStatus == DROP_TRANSFER_OUT)))
						isInterference = true;
					if (prpLc && rpLc && doesInterfere(rpLc, prpLc) && !(doesInterfere(rpLc, destPt) && (prpLc->dStatus == DROP_WAIT || prpLc->dStatus == DROP_TRANSFER_OUT)))
						isInterference = true;
...
//
//			if (dbg->x < 0 || dbg->y < 0)
//			    cout << "i=" << i << "; c=" << dbg->cycle << "; x,y=" << dbg->x << "," << dbg->y << " | ";
//		}
//		d++;
//		cout << endl;
//	}
