...
{
	// If board has already been declared, no need to re-delcare b/c it won't change
	// no matter how many assays are routed (dependent upon the architecture). Internal
	// status variables are already reset in computeIndivSubProbRotues
	if (!board)
	{
...
void RoyMazeRouter::computeIndivSupProbRoutes(vector<vector<RoutePoint *> *> *subRoutes, vector<Droplet *> *subDrops, map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	{
		if (thisTS->at(i)->GetStartTS() == startingTS) // If starting now
		{
			AssayNode *n = thisTS->at(i);

			// If this is a Transfer_Out node, then just transfer
			// the parent droplets; will not route now
			if (n->GetType() == TRANSFER_OUT)
			{
				// If transfer reference not set, then this is a normal (non-routing) DAG and we need to
				// transfer the droplet and set the transfer reference/location so that the routing-DAG
				// knows where to route to
				if (!n->GetTransferRef())
				{
			...
			else if (n->GetType() != DISPENSE && n->GetType() != TRANSFER_IN) // Otherwise, if not dispense, route it!!
				routableThisTS.push_back(n);
		}
	}
	...
	set<Droplet *> dropletsTransferredIn; // Droplets being transferred in this time-step
	vector<Droplet *> *routingThisTS = new vector<Droplet *>();
	SoukupCell *s = nullptr;
	SoukupCell *t = nullptr;
	for (unsigned i = 0; i < routableThisTS.size(); i++)
	{
		...
		for (unsigned p = 0; p < n->GetParents().size(); p++)
		{
			...
			if (par->GetType() != TRANSFER_IN)
				par->droplets.pop_back(); // TransferIn nodes will keep a copy of the droplet
			n->addDroplet(pd);
			if (n->GetReconfigMod())
			...
			else // DTG, this will need to be adjusted for storage etc., when/if more than one destination in a module
			{
				...
				else if (n->GetType() == TRANSFER_OUT && n->GetTransferRef())
					t = board->at(n->GetTransferRef()->x)->at(n->GetTransferRef()->y); // Location in next DAG where droplet is headed
				else
					t = board->at(n->GetReconfigMod()->getRX())->at(n->GetReconfigMod()->getBY()); // Bottom-Right, else
			}
			...
			else if (par->GetType() == TRANSFER_IN)
			{
				// Save droplet in set for later reference (so we know we need a source blockage)
				dropletsTransferredIn.insert(pd);

				// If assay is being compiled alone w/o the incoming routing information
				// from another assay (DAG), then create new routing point to same as target
				if (((*routes)[pd])->empty())
				{
					// If transfer reference not set, then this is a normal (non-routing) DAG and we need to
					// set the transfer reference/location for the routing-DAG to know where to route to and
					// set the local destination to the source (no routing)
					if (!par->GetTransferRef())
					{
						RoutePoint *rp = new RoutePoint();
						rp->cycle = routeCycle++;
						rp->dStatus = DROP_TRANSFER_IN;
						rp->x = t->x;
						rp->y = t->y;
					...
					else // Are in routing-DAG and need to grab source from parent's transfer location
					{
						RoutePoint *rp = new RoutePoint();
						rp->cycle = routeCycle++;
						rp->dStatus = DROP_TRANSFER_OUT;
                        rp->x = par->GetTransferRef()->x;
                        rp->y = par->GetTransferRef()->y;
						(*routes)[pd]->push_back(rp);
		...
		if (s != t)
		{
			...
			for (unsigned j = 0; j < routingThisTS->size(); j++)
			{
				...
				if (d != d2)
				{
					...
					if ( c != s && c != t) // Don't block self source
						for (int x = c->x-1; x <= c->x+1; x++)
							for (int y = c->y-1; y <= c->y+1; y++)
								if (x >= 0 && y >= 0 && x < arch->getNumCellsX() && y < arch->getNumCellsY()) // On board?
								{    //if ( !(abs(x - t->x) <= 1 && abs(y - t->y) <= 1) ) // Don't block own target
									if (!(*routes)[d2]->empty() || dropletsTransferredIn.count(d2) !=
																   0) // Don't mark as blockage for dispense droplets (but do for transfer-ins); they wait in reservoir
						...
						for (int x = c->x-1; x <= c->x+1; x++)
							for (int y = c->y-1; y <= c->y+1; y++)
								...
								{    //if ( !(abs(x - t->x) <= 1 && abs(y - t->y) <= 1) ) // Don't block self's target
									...
									if (dropsBeingOutput.count(d2) ==
										0) // Don't mark as blockage for output dorplets; they go into reservoir
										board->at(x)->at(y)->S = Blockage;
								}
	...
	// Sanity loop to make sure all TRANSFER_IN droplets start with a DROP_TRANSFER_IN status
	// and all TRANSFER_OUT droplts start with a DROP_TRANSFER_OUT status
	for (unsigned i = 0; i < thisTS->size(); i++)
	{
		if (thisTS->at(i)->GetStartTS() == startingTS) // If starting now
		{
			AssayNode *n = thisTS->at(i);
			if (n->GetType() == TRANSFER_IN)
			{
				Droplet *d = n->GetDroplets().front();
				for (int i = 0; i < subDrops->size(); i++)
				{
					if (d == subDrops->at(i))
					{
						vector<RoutePoint *> *sr = subRoutes->at(i);
						if (sr->size() > 0)
						{
						...
						//(*routes)[n->GetDroplets().front()]->front()->dStatus = DROP_TRANSFER_IN;
						break;
					}
				}
			}
			else if (n->GetType() == TRANSFER_OUT)
			{
				...
				for (int i = 0; i < subDrops->size(); i++)
				{
					if (d == subDrops->at(i))
					{
						...
						if (sr->size() > 0)
						{
							RoutePoint *rp = sr->back();
							rp->dStatus = DROP_TRANSFER_OUT;
							//cout << "Back" << endl;
						}
						//(*routes)[n->GetDroplets().front()]->back()->dStatus = DROP_TRANSFER_OUT;
						break;
					}
				}
			}
