...
string Analyze::AnalyzeSchedule(DAG *dag, DmfbArch *arch, Scheduler *scheduler)
{
	...
	rs << "Number Heating Nodes: " << dag->heats.size() << endl;
	rs << "Number Cooling Nodes: " << dag->cools.size() << endl;
	...
	for (unsigned i = 0; i < dag->cools.size(); i++)
	{
		AssayNode *c = dag->cools.at(i);
	...
	for (unsigned i = 0; i < dag->allNodes.size(); i++)
	{
		...
		if (!(n->type == DISPENSE || n->type == OUTPUT || n->type == TRANSFER_IN || n->type == TRANSFER_OUT))
		{
			if (n->type == STORAGE)
			{
...
string Analyze::AnalyzePlacement(DmfbArch *arch, vector<ReconfigModule *> *rModules)
{
	...
	// Final output
	if (outputWarning)
		cout << endl << "Discovered several warnings; please check the output file for more details." << endl << "Done." << endl;
...
string Analyze::AnalyzeDropletConcentrationAndIO(DAG *dag, DmfbArch *arch, map<Droplet *, vector<RoutePoint *> *> *routes)
{
	...
	for (; routeIt != routes->end(); routeIt++)
	{
		...
		// Determine if the droplet was terminated properly (output or merged or transferred out)
		bool properlyTerminated = false;
		if (lastRp->dStatus == DROP_OUTPUT || lastRp->dStatus == DROP_MERGING || lastRp->dStatus == DROP_TRANSFER_OUT)
...
bool Analyze::DoesInterfere(RoutePoint *rp1, RoutePoint *rp2)
{
	if (!rp1 || !rp2) // If either is null, then assume error already reported and don't report as interference error
		...
		return false;
	else if (rp1->dStatus == DROP_MERGING || rp2->dStatus == DROP_MERGING || rp1->dStatus == DROP_SPLITTING || rp2->dStatus == DROP_SPLITTING
				|| rp1->dStatus == DROP_TRANSFER_IN_MERGE || rp2->dStatus == DROP_TRANSFER_IN_MERGE) // In interference region but marked as splitting/merging
