...
void FileOut::WriteDagToFile(DAG *dag, string fileName)
{
	...
	for (unsigned i = 0; i < dag->getAllNodes().size(); i++)
	{
		...
		else if (node->GetType() == COOL)// Id, Type, Time (s), Name
			os << node->id << ", COOL, " << node->seconds << ", " << node->name << ")\n";
		...
		else if (node->GetType() == TRANSFER_IN) // Id, Type, Name
			os << node->id << ", TRANSFER_IN, " << node->name << ")\n";
		else if (node->GetType() == TRANSFER_OUT) // Id, Type, Name
			os << node->id << ", TRANSFER_OUT, " << node->name << ")\n";
		else // Id, Type, Name
...
void FileOut::WriteCfgToFile(CFG *cfg, string directory, string fileName)
{
	...
	{
		stringstream str;
		str << "Failed to properly write CFG to file: " << fileName << endl;
		claim (os.good(), &str);
	}
	...
	// Output the number of conditional groups
	// NUMCGS(numConditionalGroups)
	os << "NUMCGS(" << cfg->conditionalGroups.size() << ")" << endl << endl;
	...
	for (int i = 0; i < cfg->conditionalGroups.size(); i++)
	{
		ConditionalGroup *cg = cfg->conditionalGroups.at(i);
		...
		for (int j = 0; j < cg->getConditions()->size(); j++)
		{
			...
			// Output Expressions
			stringstream ss;
			RecursiveExpressionGen(c->statement, &ss);
			os << ss.str();

			////////////////////////////////////////////
			// Output Transfer Droplets (if any)
			// TD(transOutDag, transOutNodeId, transInDag, transInNodeId)
			if (c->transfers.size() > 0)
			{
				for (int k = 0; k < c->transfers.size(); k++)
				{
...
////////////////////////////////////////////////////////////////////////////
// Ussed by the WriteCfgToFile() method to recursively generate lines/tags
// to print to file which describe a possibly-nested Expression object.
////////////////////////////////////////////////////////////////////////////
void FileOut::RecursiveExpressionGen(Expression *e, stringstream *ss)
{
	// Vector of Strings for output: should be 1-1 with ExOperationType & ExOperandType from enum.h
	string exOperationTypes[] = { "GT", "GoE", "LT", "LoE", "EQUAL", "NOT", "AND", "OR", "UNCOND" };
	string exOperandTypes[] = { "SUB_EXP", "RUN_COUNT", "ONE_SENSOR", "TWO_SENSORS", "TRUE", "FALSE" };

	// Now print out all of the expressions recursively
	// (if there is an expression/statement)
	if (e)
	{
		*ss << "\t\tEXP(" << e->getId();
		...
		else if (e->operationType == OP_AND || e->operationType == OP_OR)
		{
			...
			*ss << ", " << exOperationTypes[e->operationType];
			...
			// Finish current expression by printing out the IDs of the
			// sub-expressions before recursively diving into the sub-expressions
			for (int i = 0; i < e->operands->size(); i++)
				*ss << ", " << e->operands->at(i)->getId();
			*ss << ")" << endl;

			for (int i = 0; i < e->operands->size(); i++)
				RecursiveExpressionGen(e->operands->at(i), ss);
		}
		else if (e->operationType == OP_NOT)
		{
			...
			*ss << ", " << exOperationTypes[e->operationType];
			...
			// Finish current expression by printing out the IDs of the
			// sub-expressions before recursively diving into the sub-expressions
			*ss << ", " << e->operands->front()->getId() << ")" << endl;;
	...
	void FileOut::WriteScheduledDagAndArchToFile(DAG *dag, DmfbArch *arch, Scheduler *scheduler, string fileName)
	{
		...
		for (unsigned i = 0; i < d->getAllNodes().size(); i++)
		{
			...
			else if (node->GetType() == COOL)// Id, Type, Time (s), Name, StartTS, EndTS, ResourceType
				os << node->id << ", COOL, " << node->seconds << ", " << node->name << ", " << node->startTimeStep << ", " << node->endTimeStep << ", " << (int)node->boundedResType << ")\n";
			...
			else if (node->GetType() == TRANSFER_IN) // Id, Type, Name, StartTS, EndTS
				os << node->id << ", TRANSFER_IN, " << node->name << ", " << node->startTimeStep << ", " << node->endTimeStep << ")\n";
			else if (node->GetType() == TRANSFER_OUT) // Id, Type, Name, StartTS, EndTS
				os << node->id << ", TRANSFER_OUT, " << node->name << ", " << node->startTimeStep << ", " << node->endTimeStep << ")\n";
	...
	void FileOut::WritePlacedDagAndArchToFile(DAG *dag, DmfbArch *arch, Placer *placer, vector<ReconfigModule *> *rModules, string fileName)
	{
		...
		for (unsigned i = 0; i < d->getAllNodes().size(); i++)
		{
			...
			else if (node->GetType() == COOL)// Id, Type, Time (s), Name, StartTS, EndTS, ReconfigModId
				os << node->id << ", COOL, " << node->seconds << ", " << node->name << ", " << node->startTimeStep << ", " << node->endTimeStep << ", " << node->reconfigMod->id << ")\n";
	...
	void FileOut::WriteRoutedDagAndArchToFile(DAG *dag, DmfbArch *arch, Router *router, vector<ReconfigModule *> *rModules, map<Droplet *, vector<RoutePoint *> *> *routes, vector<vector<RoutePoint*> *> *dirtyCells, vector<vector<int> *> *pinActivations, vector<unsigned long long> *tsBeginningCycle, string fileName)
	{
		...
		for (unsigned i = 0; i < rModules->size(); i++)
		{
			...
			else if (rm->boundNode->type == HEAT || rm->boundNode->type == COOL)
	...
	///////////////////////////////////////////////////////////////
	void FileOut::WriteCompactedRoutesToFile(DAG *dag, DmfbArch *arch, vector<ReconfigModule *> *rModules, map<Droplet *, vector<RoutePoint *> *> *routes, vector<unsigned long long> *tsBeginningCycle, string fileName, CFG *cfg)
	{
		ofstream os;
		os.open(fileName.c_str());
		...
		for (DAG *d : cfg->allDags) {
            ...
            for (unsigned i = 0; i < d->getAllNodes().size(); i++) {
                ...
                if (node->GetType() == DISPENSE) // Id, Type, FluidName, Volume, Name, StartTS, EndTS, IoPortId
                    os << node->id << ", DISPENSE, " << node->portName << ", " << node->volume << ", " << node->name
                       << ", " << node->startTimeStep << ", " << node->endTimeStep << ", " << node->ioPort->id << ")\n";
                else if (node->GetType() ==
                         MIX) // Id, Type, NumDropsBefore, Time (s), Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", MIX, " << node->numDrops << ", " << node->seconds << ", " << node->name << ", "
                       << node->startTimeStep << ", " << node->endTimeStep << ", " << rmId << ")\n";
                else if (node->GetType() ==
                         DILUTE) // Id, Type, NumDropsBefore, Time (s), Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", DILUTE, " << node->numDrops << ", " << node->seconds << ", " << node->name
                       << ", " << node->startTimeStep << ", " << node->endTimeStep << ", " << rmId << ")\n";
                else if (node->GetType() ==
                         SPLIT) // Id, Type, NumDropsAfter, Time (s), Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", SPLIT, " << node->numDrops << ", " << node->seconds << ", " << node->name
                       << ", " << node->startTimeStep << ", " << node->endTimeStep << ", " << rmId << ")\n";
                else if (node->GetType() == HEAT)// Id, Type, Time (s), Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", HEAT, " << node->seconds << ", " << node->name << ", " << node->startTimeStep
                       << ", " << node->endTimeStep << ", " << rmId << ")\n";
                else if (node->GetType() == COOL)// Id, Type, Time (s), Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", COOL, " << node->seconds << ", " << node->name << ", " << node->startTimeStep
                       << ", " << node->endTimeStep << ", " << rmId << ")\n";
                else if (node->GetType() ==
                         DETECT) // Id, Type, NumDropsIo, Time (s), Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", DETECT, " << node->numDrops << ", " << node->seconds << ", " << node->name
                       << ", " << node->startTimeStep << ", " << node->endTimeStep << ", " << rmId << ")\n";
                else if (node->GetType() == OUTPUT) // Id, Type, SinkName, Name, StartTS, EndTS, IoPortId
                    os << node->id << ", OUTPUT, " << node->portName << ", " << node->name << ", "
                    ...
                    os << node->id << ", STORAGE, " << node->startTimeStep << ", " << node->endTimeStep << ", "
                       << node->reconfigMod->id << ")\n";
                else if (node->GetType() == TRANSFER_IN)// Id, Type, Name, StartTS, EndTS
                    os << node->id << ", TRANSFER_IN, " << node->name << ", " << node->startTimeStep << ", "
                       << node->endTimeStep << ")\n";
                else if (node->GetType() == TRANSFER_OUT)// Id, Type, Name, StartTS, EndTS
                    os << node->id << ", TRANSFER_OUT, " << node->name << ", " << node->startTimeStep << ", "
                       << node->endTimeStep << ")\n";
                else // Id, Type, Name, StartTS, EndTS, ReconfigModId
                    os << node->id << ", GENERAL, " << node->name << ", " << node->startTimeStep << ", "
                       << node->endTimeStep << ", " << node->reconfigMod->id << ")\n";
