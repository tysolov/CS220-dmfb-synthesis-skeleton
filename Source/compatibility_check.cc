...
void CompatChk::PreScheduleChk(Scheduler *s, DmfbArch *arch, DAG *dagToSchedule, bool runAsEntireFlow)
{
	...
	else if (s->getType() == ROUTING_BASED_SYNTHESIS_S)
	...
	{
		stringstream ss("");
		ss << "The assay requires heaters/coolers, but the supplied architecure file (" << arch->getName() << ") has none.";
...
void CompatChk::PrePlaceChk(Placer *p, DmfbArch *arch, bool runAsEntireFlow)
{
	...
	if (p->getType() == ROUTING_BASED_SYNTHESIS_P)
	{
		...
		claim(p->getPastSchedType() == ROUTING_BASED_SYNTHESIS_S, "The routing-based-synthesis placer must be paired with the routing-based-synthesis scheduler.");
	}
...
void CompatChk::PreRouteChk(Router *r, DmfbArch *arch, bool runAsEntireFlow)
{
	...
	else if (r->getType() == ROUTING_BASED_SYNTHESIS_R)
	{
		...
		claim(r->getPastSchedType() == ROUTING_BASED_SYNTHESIS_S, "The routing-based-synthesis router must be paired with the routing-based-synthesis scheduler.");
		claim(r->getPastPlacerType() == ROUTING_BASED_SYNTHESIS_P, "The routing-based-synthesis router must be paired with the routing-based-synthesis placer.");
	...
	if (r->performWash())
	{
		...
		washableTypes.insert(ROUTING_BASED_SYNTHESIS_R);
...
{
	if(r->getType() == FPPC_SEQUENTIAL_R || r->getType() == FPPC_PARALLEL_R || r->getType() == ROUTING_BASED_SYNTHESIS_R)
		return false;
	else
		return true;
}
...
{
	if(r->getType() == ROUTING_BASED_SYNTHESIS_R)
		return false;
	else
		return true;
}
